...
<body>
	...
	<%	User user2 = (User) session.getAttribute("userObj");
		...
		List<Notes> notes = ndao.fetchNotesByUserId(user2.getId());
		if (!notes.isEmpty()) { %>
			<div class="container-fluid px-3 py-4" id="featured-3">
				<h2 class="pb-2 m-0 border-bottom count">TOTAL NOTES : <%= notes.size() %></h2>
				<div class="d-flex justify-content-between ps-4 py-2 btn_grp">
					<div class="d-grid gap-2 d-md-block">
						<a href="notes.jsp" class="btn btn-success disabled">All Notes</a>
						<a href="addnote.jsp" class="btn btn-primary me-md-2">Add Note</a>
					</div>
					<form class="d-grid gap-2 d-md-flex justify-content-md-end" action="notebytitle.jsp">
						<input class="form-control me-2" type="search" placeholder="Enter Title Here" aria-label="Search" name="mytitle">
		...
		<% for (Notes note : notes) { %>
		<div class="modal fade" id="editModal<%= note.getId() %>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<form class="px-3 pt-3" action="editnote" method="post">
						...
						<div class="mb-2">
							<label class="form-label">Description :</label> <input name="desc" type="text" value="<%= note.getContent() %>" class="form-control" required>
						</div>
